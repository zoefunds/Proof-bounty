...
| `INSUFFICIENT_EVIDENCE` is a distinct outcome from `REJECTED`: exhausting its resubmission cycle *refunds* the bond instead of forfeiting it, and does not penalize reputation | `request_verification`'s `VERDICT_INSUFFICIENT_EVIDENCE` branch, `ATTEMPT_INSUFFICIENT_EVIDENCE_FINAL` | `test_human_override_flagged_and_counted_when_no_prior_ai_verdict_exists` and related direct-mode tests exercise the surrounding state machine; the verdict itself observed live in `scripts/10` |
...
| Both `resolve_dispute` and `resolve_appeal` reject a bare verdict with no written justification | `resolution_note` non-empty check in both methods | `test_resolve_dispute_rejects_empty_resolution_note`, `test_resolve_appeal_rejects_empty_resolution_note` |
| An arbiter's ruling never moves money immediately — it opens a 2-day appeal window, executed only by a separate, permissionless `finalize_arbiter_resolution` call once that window closes | `resolve_dispute` sets `ATTEMPT_ARBITER_RESOLVED_PENDING_APPEAL` + `appeal_deadline`; `finalize_arbiter_resolution`'s `self._now() < appeal_deadline` guard | `test_zero_bond_reclaim_after_settlement_rejects_before_appeal_window` (finalize correctly rejected before the window closes); `test_zero_bond_reclaim_after_settlement_is_a_safe_noop` (finalize correctly succeeds after, via `gltest.direct`'s time-warp) |
| Whether a human ruling actually **changed** the outcome (vs. merely confirming AI's own verdict) is computed and recorded per-attempt, not asserted | `_human_verdict_matches_ai_outcome`, `Attempt.human_verdict_overrode_ai`, set in `resolve_dispute` and re-evaluated in `resolve_appeal` | `test_human_override_flagged_and_counted_when_no_prior_ai_verdict_exists` (no prior AI verdict → correctly flagged as override), `test_human_override_not_flagged_when_arbiter_agrees_with_ai_verdict` (arbiter agrees with AI → correctly NOT flagged as override) |
| The real, contract-wide rate of human overrides vs. AI-decided outcomes is a always-queryable on-chain fact | `attempts_settled_by_ai_consensus`, `attempts_settled_by_human_override`, `get_settlement_transparency()`, incremented via `_record_settlement_provenance` at every terminal settlement point (`request_verification`, `claim_bond_forfeiture`, `finalize_arbiter_resolution`, `resolve_appeal`) | `test_human_override_flagged_and_counted_when_no_prior_ai_verdict_exists` asserts the counters directly (`get_settlement_transparency()` before/after) |
| `resolve_appeal` is gated to the protocol owner specifically, not merely "not a stranger" | `_require_owner()` in `resolve_appeal` | `test_resolve_appeal_rejects_non_owner` (the arbiter — a real, non-stranger party — is correctly rejected) |
| The owner's non-monetary admin surface (fee, treasury, pause) is a materially smaller, already-disclosed trust boundary than a new unstaked authority would be — and is now honestly documented as including `resolve_appeal`, not excluding it | `owner: Address` field docstring | N/A — documentation correctness, not a runtime invariant; verify by reading the field docstring directly |
...
| Only the protocol owner may `resolve_appeal`, pause the protocol, or change the fee/treasury | `_require_owner` | `test_resolve_appeal_rejects_non_owner`, `test_admin_update_fee_rejects_non_owner` |
...
- **A full real-browser-wallet end-to-end test** (MetaMask/WalletConnect popup approval, captured screenshots across every UI state) has not been run. The transaction-lifecycle state machine (`lib/use-contract-write.ts`) is exercised by every write path described above through direct contract calls, not through a driven browser session.
