...
| The arbiter's ruling never has the final word — appealing it always escalates to a second, independent round of GenLayer consensus, never to a human's judgment | `resolve_appeal` is fully permissionless (no `_require_owner`/`_require_arbiter` gate at all) and decided by `_collect_appeal_verdict`, a real `gl.nondet.web.render` + `gl.eq_principle.prompt_comparative` round, not a human-supplied verdict | `test_resolve_appeal_is_decided_by_genlayer_consensus_not_a_human` (`@pytest.mark.llm`, real web fetch + real LLM consensus, called by the challenger themselves — no owner key anywhere in the test); live via `scripts/09-appeal-flow.mjs` and `scripts/12-product-test-c-dispute-appeal.mjs` |
...
| An arbiter's ruling never moves money immediately — it opens a 2-day appeal window, executed only by a separate, permissionless `finalize_arbiter_resolution` call once that window closes (if nobody appealed) | `resolve_dispute` sets `ATTEMPT_ARBITER_RESOLVED_PENDING_APPEAL` + `appeal_deadline`; `finalize_arbiter_resolution`'s `self._now() < appeal_deadline` guard | `test_zero_bond_reclaim_after_settlement_rejects_before_appeal_window` (finalize correctly rejected before the window closes); `test_zero_bond_reclaim_after_settlement_is_a_safe_noop` (finalize correctly succeeds after, via `gltest.direct`'s time-warp) |
| Whether an UNAPPEALED arbiter ruling actually **changed** the outcome (vs. merely confirming AI's own verdict) is computed and recorded per-attempt, not asserted; a `resolve_appeal` resolution is never counted as a "human override" since GenLayer consensus, not a human, decided it | `_human_verdict_matches_ai_outcome`, `Attempt.human_verdict_overrode_ai` (set in `resolve_dispute`, left untouched by `resolve_appeal`); `_record_settlement_provenance(..., via_human_ruling=False)` in `resolve_appeal` vs. `via_human_ruling=True` only in `finalize_arbiter_resolution` | `test_human_override_flagged_and_counted_when_no_prior_ai_verdict_exists`, `test_human_override_not_flagged_when_arbiter_agrees_with_ai_verdict` |
| The real, contract-wide rate of (unappealed) human overrides vs. AI-consensus-decided outcomes (which includes every `resolve_appeal` resolution) is an always-queryable on-chain fact | `attempts_settled_by_ai_consensus`, `attempts_settled_by_human_override`, `get_settlement_transparency()` | `test_human_override_flagged_and_counted_when_no_prior_ai_verdict_exists` asserts the counters directly; `test_resolve_appeal_is_decided_by_genlayer_consensus_not_a_human` asserts a `resolve_appeal` resolution increments the AI-consensus counter, not the human-override one |
