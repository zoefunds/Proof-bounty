...
| | |
|---|---|
| **Live contract address** | `0x4b8b06e93aD3e06F29a4491844904743B6d9a0b2` (4th deployment, 2026-08-26) |
| **Does the LIVE bytecode match `contracts/proof_bounty.py` right now?** | **Yes, confirmed directly against the deployed bytecode, not assumed: `genlayer code 0x4b8b06e93aD3e06F29a4491844904743B6d9a0b2` was fetched and grepped for `INSUFFICIENT_EVIDENCE` — 13 matches, meaning THIS deployment genuinely includes the fix the previous address (`0x9A2b...`) was missing. Also confirmed reachable via a real `get_bounty_counter` read (returned 0 — fresh contract, no state yet).** |
